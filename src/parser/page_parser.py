"""HTML parser for SEO, resources, emails, and social media."""
...
from src.models.entities import ImageInfo, LinkInfo, PageData, ResourceInfo
from src.utils.url_tools import normalize_url, same_domain
...
class _SEOHTMLParser(HTMLParser):
    """Small HTML parser that extracts crawl facts without crashing."""
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if tag == "link" and attr.get("href"):
            ...
            if "canonical" in rel:
                self.canonical = href
            kind = "CSS" if "stylesheet" in rel else "RSS" if "alternate" in rel else "Font"
        ...
        if tag == "a" and attr.get("href") and not attr["href"].startswith(("mailto:", "tel:", "javascript:")):
            ...
            if target.lower().endswith(".pdf"):
                self.resources.append(ResourceInfo(self.page_url, target, "PDF"))
        if tag == "img" and attr.get("src"):
            source = attr["src"]
            self.images.append(ImageInfo(self.page_url, normalize_url(source, self.page_url), attr.get("alt", ""), self._int(attr.get("width")), self._int(attr.get("height")), format=urlparse(source).path.rsplit(".", 1)[-1].lower(), lazy_load=attr.get("loading") == "lazy"))
...
class PageParser:
    """Parse an HTML response into a PageData object."""
    ...
    def parse(
        ...
        return PageData(
            ...
            internal_links_count=sum(1 for link in parser.links if link.is_internal),
            external_links_count=sum(1 for link in parser.links if not link.is_internal),
            ...
            social_profiles=self._social_profiles(parser.links),
        )

    def _social_profiles(self, links: list[LinkInfo]) -> dict[str, set[str]]:
        ...
        for link in links:
            ...
            for platform, domain in SOCIAL_HOSTS.items():
                if domain in host:
                    profiles[platform].add(link.target_url)
