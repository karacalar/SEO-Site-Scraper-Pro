...
def test_parser_extracts_core_seo_entities() -> None:
    html = """
    <html lang='en'><head><title>Example Title</title><meta name='description' content='A useful page description for testing.'>
    <link rel='canonical' href='/canonical'><meta property='og:title' content='OG'><meta name='twitter:card' content='summary'>
    <script type='application/ld+json'>{}</script></head><body><h1>Main</h1><h2>Sub</h2>
    <a href='/about' rel='nofollow sponsored'>About</a><a href='https://github.com/example'>GitHub</a>
    <img src='/logo.png' loading='lazy'><p>Contact hello@example.com with enough visible words for parsing.</p></body></html>
    """
...
def test_analyzer_scores_missing_metadata() -> None:
    ...
    result = CrawlResult(CrawlSettings("http://example.com/"), pages={page.url: page}, images=page.images, links=page.links)
    ...
    assert "Page is not served over HTTPS" in descriptions
